...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.socialmedias.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.socialmedias.store') }}">
                @csrf
...
<script>
    Dropzone.options.imageDropzone = {
        ...
        init: function () {
            @if(isset($socialmedias) && $socialmedias->image)
                var file = {!! json_encode($socialmedias->image) !!}
