...
@section('content')
    @can('order_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.orderbarangs.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.orderbarangs.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover datatable datatable-Order">
                    <thead>
                        <tr>
                            <th width="10"></th>
                            <th>{{ trans('cruds.orderbarangs.fields.id') }}</th>
                            <th>Kode Invoice</th>
                            <th>{{ trans('cruds.orderbarangs.fields.nama_pemesan') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.cabang') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.alamat') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.product') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.price') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.start_date') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.end') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.status_bayar') }}</th>
                            <th>{{ trans('cruds.orderbarangs.fields.bukti_pembayaran') }}</th>
                            <th>&nbsp;</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @foreach ($orderbarangs as $order)
                            <tr data-entry-id="{{ $order->id }}">
                                ...
                                <!-- Tanggal Mulai -->
                                <td class="text-center">{{ $order->jatuh_tempo ?? '' }}</td>
                                ...
                                <td>
                                    @can('order_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.orderbarangs.show', $order->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('order_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.orderbarangs.edit', $order->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('order_delete')
                                        <form action="{{ route('admin.orderbarangs.destroy', $order->id) }}" method="POST" class="delete-form" style="display: inline-block;">
