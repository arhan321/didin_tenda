...
@section('content')
    @can('deliveryorder_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.deliveryordertech.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.deliveryordertech.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.deliveryordertech.title_singular') }} {{ trans('global.list') }}
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover datatable datatable-DeliveryOrder">
                    <thead>
                        <tr>
                            <th width="10"></th>
                            <th>{{ trans('cruds.deliveryordertech.fields.id') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.nama_pemesan') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.cabang') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.alamat') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.product') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.status') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.tanggal') }}</th>
                            <th>{{ trans('cruds.deliveryordertech.fields.pengantar') }}</th>
                            <th>&nbsp;</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        @foreach ($deliveryordertechs as $deliveryOrder)
                            <tr data-entry-id="{{ $deliveryOrder->id }}">
                                ...
                                <td>
                                    @can('deliveryorder_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.deliveryordertech.show', $deliveryOrder->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('deliveryorder_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.deliveryordertech.edit', $deliveryOrder->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('deliveryorder_delete')
                                        <form action="{{ route('admin.deliveryordertech.destroy', $deliveryOrder->id) }}" method="POST" class="delete-form" style="display: inline-block;">
        ...
        $(function() {
            ...
            @can('deliveryorder_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('admin.deliveryordertech.massDestroy') }}",
