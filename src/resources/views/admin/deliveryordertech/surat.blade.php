...
<div class="modal fade" id="invoiceModal" tabindex="-1" role="dialog" aria-labelledby="invoiceModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content" style="background-image: url('{{ asset('assets/sahabatech5.png') }}'); background-size: 900px; background-repeat: no-repeat; background-position: center;">
        ...
        <div class="modal-body" id="invoiceContent">
          ...
          <div style="font-family: Arial, sans-serif; font-size: 14px; padding: 20px;">
              ...
              <div class="paket-tulisan" style="margin-top: -20px;">
                <div style="display: flex; align-items: center;">
                  <div style="margin-right: 10px; margin-bottom: 19px;">
                    <img src="{{ asset('assets/image_sahabatech.png') }}" alt="Logo" style="width: 35px; height: auto;">
                  </div>
                  <div>
                    <h2 style="color: #0033a0; margin: 0; font-size: 32px; margin-bottom: 65px; margin-top: 65px; font-family: 'Montserrat', sans-serif;">SAHABAT TECH</h2>
                    <p style="margin: 0; margin-left: -40px; margin-top: -40px; font-size: 15px; font-family: 'Montserrat', sans-serif;">
                      Jl. KH. Mas Mansyur No.28, RT.PinangRT.005/RW.003,
                      Sudimara Pinang, <br>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                  ...
                  <tr>
                    <td style="padding: 5px;">
                        {{ \Carbon\Carbon::parse($deliveryordertech->tanggal_pengiriman)->format('d/m/Y') }}
                    </td>
                    <td style="padding: 5px; ">
                      {{ $deliveryordertech->id }}-{{ \Carbon\Carbon::parse($deliveryordertech->created_at)->format('m') }}-ST-{{ \Carbon\Carbon::parse($deliveryordertech->created_at)->format('Y') }}
                    </td>
                </tr>
                </table>
              </div>
            ...
            <div style="margin-top: 20px;">
              ...
              <p style="padding-left: 10px; margin-top: 5px;">
                  <strong>{{ $deliveryordertech->client->nama_client ?? 'Unknown' }}</strong><br>
                  {{ $deliveryordertech->client->alamat_client ?? 'Alamat tidak tersedia' }}
              </p>
          </div>
          ...
          {{-- <div class="tulisan"
              ...
              <p>SURAT JALAN PERIODE
                  @php
                      $formattedDate = \Carbon\Carbon::parse($deliveryordertech->start)->format(' F Y');
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
              ...
              <tbody>
                  @php
                      $productDetails = isset($deliveryordertech->product_details)
...
<script>
  ...
  const deliveryOrderData = {
            id: {{ $deliveryordertech->id }},
            createdAt: "{{ $deliveryordertech->created_at }}"
        };
        ...
        const invoiceNumber = `${deliveryOrderData.id}-${month}-SJ-TAP-${year}`;
