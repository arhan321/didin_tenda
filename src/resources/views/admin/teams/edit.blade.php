        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.teams.update', [$team->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}"
                        name="description" id="description">{{ old('description', $team->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" name="position" id="position">
                    ...
                    <option value="manager" {{ old('position', $team->position) === 'manager' ? 'selected' : '' }}>Manager</option>
                    <option value="staff" {{ old('position', $team->position) === 'staff' ? 'selected' : '' }}>Staff</option>
                    <option value="admin" {{ old('position', $team->position) === 'admin' ? 'selected' : '' }}>Admin</option>
                    <option value="direktur" {{ old('position', $team->position) === 'direktur' ? 'selected' : '' }}>Direktur</option>
                    <option value="cto" {{ old('position', $team->position) === 'cto' ? 'selected' : '' }}>CTO</option>
                    <option value="owner" {{ old('position', $team->position) === 'owner' ? 'selected' : '' }}>Owner</option>
                    <option value="it_support" {{ old('position', $team->position) === 'it_support' ? 'selected' : '' }}>IT Support</option>
                </select>
