...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.dodgpros.update', [$dodgpros->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('pengantar') ? 'is-invalid' : '' }}" 
                           ...
                           value="{{ old('pengantar', $dodgpros->pengantar) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('tanggal_pengiriman') ? 'is-invalid' : '' }}" 
                           ...
                           value="{{ old('tanggal_pengiriman', $dodgpros->tanggal_pengiriman) }}">
                    ...
                    <select class="form-control {{ $errors->has('client_id') ? 'is-invalid' : '' }}" 
                        ...
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}"
                                {{ old('client_id', $dodgpros->client_id) == $client->id ? 'selected' : '' }}>
                                {{ $client->nama_client }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}"
                                {{ in_array($product->id, old('products', [])) || collect($dodgpros->product_details ?? [])->pluck('id')->contains($product->id) ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" 
                        ...
                        <option value="pending" {{ old('status', $dodgpros->status) == 'pending' ? 'selected' : '' }}>
                            pending
                        </option>
                        <option value="delivered" {{ old('status', $dodgpros->status) == 'delivered' ? 'selected' : '' }}>
                            delivered
                        </option>
                        <option value="canceled" {{ old('status', $dodgpros->status) == 'canceled' ? 'selected' : '' }}>
                            canceled
                        </option>
                    </select>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Mengambil kuantitas lama, default diambil dari dodgpros->product_details (pluck qty berdasarkan id)
            const oldQuantities = @json(old('product_qty', $dodgpros->product_details->pluck('qty', 'id')->toArray()));
            ...
            function updateProductQuantities() {
                ...
                selectedProducts.forEach(function(productId) {
                    ...
                    productQuantitiesContainer.append(`
                        <div class="form-group">
                            <label for="qty_${productId}">${productLabel} {{ trans('cruds.dodgpros.fields.qty') }}</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const selectedClientId = "{{ old('client_id', $dodgpros->client_id) }}";
            const selectedBranchId = "{{ old('branch_id', $dodgpros->branch_id) }}";
            const selectedAlamatId = "{{ old('alamat_id', $dodgpros->alamat_id) }}";
