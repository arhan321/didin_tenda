...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.deliveryorderbarang.update', [$deliveryorderbarang->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('pengantar') ? 'is-invalid' : '' }}" 
                           ...
                           value="{{ old('pengantar', $deliveryorderbarang->pengantar) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('tanggal_pengiriman') ? 'is-invalid' : '' }}" 
                           ...
                           value="{{ old('tanggal_pengiriman', $deliveryorderbarang->tanggal_pengiriman) }}">
                    ...
                    <select class="form-control {{ $errors->has('client_id') ? 'is-invalid' : '' }}" 
                        ...
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}"
                                {{ old('client_id', $deliveryorderbarang->client_id) == $client->id ? 'selected' : '' }}>
                                {{ $client->nama_client }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}"
                                {{ in_array($product->id, old('products', [])) || collect($deliveryorderbarang->product_details ?? [])->pluck('id')->contains($product->id) ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" 
                        ...
                        <option value="pending" {{ old('status', $deliveryorderbarang->status) == 'pending' ? 'selected' : '' }}>
                            pending
                        </option>
                        <option value="delivered" {{ old('status', $deliveryorderbarang->status) == 'delivered' ? 'selected' : '' }}>
                            delivered
                        </option>
                        <option value="canceled" {{ old('status', $deliveryorderbarang->status) == 'canceled' ? 'selected' : '' }}>
                            canceled
                        </option>
                    </select>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            // Mengambil harga produk dari ProductBarang
            const productPrices = @json($productPrices);
            // Mengambil kuantitas lama, default diambil dari deliveryorderbarang->product_details (pluck qty berdasarkan id)
            const oldQuantities = @json(old('product_qty', $deliveryorderbarang->product_details->pluck('qty', 'id')->toArray()));
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const selectedClientId = "{{ old('client_id', $deliveryorderbarang->client_id) }}";
            const selectedBranchId = "{{ old('branch_id', $deliveryorderbarang->branch_id) }}";
            const selectedAlamatId = "{{ old('alamat_id', $deliveryorderbarang->alamat_id) }}";
