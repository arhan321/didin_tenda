...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.sahabatechinvoice.title_singular') }}
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.invoices.update', [$invoice->id]) }}" enctype="multipart/form-data">
                ...
                <!-- Dropdown Branch -->
                <div class="form-group">
                    <label for="branch_id">{{ trans('cruds.sahabatechinvoice.fields.cabang') }}</label>
                    ...
                    @endif
                    <span class="help-block">{{ trans('cruds.sahabatechinvoice.fields.cabang_helper') }}</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    @endif
                    <span class="help-block">{{ trans('cruds.sahabatechinvoice.fields.alamat_helper') }}</span>
                </div>



                <div class="form-group">
                    <label class="required" for="products">{{ trans('cruds.sahabatechinvoice.fields.product') }}</label>
                    ...
                    <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}"
                                {{ in_array($product->id, old('products', $invoice->product_details->pluck('id')->toArray())) ? 'selected' : '' }}>
                                {{ $product->name }} 
                            </option>
                        @endforeach
                    </select>
                    ...
                    @endif
                    <span class="help-block">{{ trans('cruds.sahabatechinvoice.fields.product_helper') }}</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number"
                        name="price" id="price" value="{{ old('price', $invoice->price) }}" step="0.01">
                    @if ($errors->has('price'))
                        <div class="invalid-feedback">
                            {{ $errors->first('price') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.sahabatechinvoice.fields.total_price_helper') }}</span>
                </div>


                <div class="form-group">
                    <label class="required" for="start">{{ trans('cruds.sahabatechinvoice.fields.start') }}</label>
                    <input class="form-control {{ $errors->has('start') ? 'is-invalid' : '' }}" type="date"
                        name="start" id="start" value="{{ old('start', $invoice->start) }}" required>
                    @if ($errors->has('start'))
                        <div class="invalid-feedback">
                            {{ $errors->first('start') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.sahabatechinvoice.fields.start_helper') }}</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status_bayar') ? 'is-invalid' : '' }}" name="status_bayar"
                        ...
                        <option value disabled
                            {{ old('status_bayar', $invoice->status_bayar) === null ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}</option>
                        @foreach (App\Models\Order::STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('status_bayar', $invoice->status_bayar) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="metode_pembayaran" id="metode_upload"
                        value="upload"
                        {{ old('metode_pembayaran', $invoice->metode_pembayaran) == 'upload' ? 'checked' : '' }}>
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="metode_pembayaran" id="metode_cash" value="cash"
                        {{ old('metode_pembayaran', $invoice->metode_pembayaran) == 'cash' ? 'checked' : '' }}>
                ...
                <div class="form-group" id="upload_bukti_pembayaran" style="display: none;">
                    ...
                    <div class="custom-file">
                        ...
                        <label class="custom-file-label" id="fileLabel" for="bukti_pembayaran_file">
                            ...
                            {{ old('bukti_pembayaran', $invoice->bukti_pembayaran ? basename($invoice->bukti_pembayaran) : 'Pilih file PDF...') }}
                        </label>
                    </div>
                </div>
                ...
                @if (isset($invoice) && $invoice->bukti_pembayaran && $invoice->bukti_pembayaran !== 'CASH')
                    <div class="form-group">
...
$(document).ready(function() {
    ...
    const oldQuantities = @json(old('product_qty', $invoice->product_details->pluck('qty', 'id')->toArray()));
    ...
    function updateProductQuantities() {
        ...
        selectedProducts.forEach(function(productId) {
            ...
            productQuantitiesContainer.append(`
                <div class="form-group">
                    <label for="qty_${productId}">${productLabel} {{ trans('cruds.sahabatechinvoice.fields.qty') }}</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Tanggal end dari invoice, pastikan value diambil dari variabel $invoice->end
            var endDate = "{{ $invoice->end }}";
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            "{{ old('client_id', $invoice->client_id) }}"; // Ambil client_id yang sudah disimpan sebelumnya
            var selectedBranchId =
            "{{ old('branch_id', $invoice->cabang_id) }}"; // Ambil branch_id yang sudah disimpan sebelumnya
            var selectedAlamatId =
            "{{ old('alamat_id', $invoice->alamat_id) }}"; // Ambil alamat_id yang sudah disimpan sebelumnya
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingBuktiPembayaran = "{{ $invoice->bukti_pembayaran }}"; // Ambil data dari PHP
