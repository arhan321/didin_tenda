    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.addons.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">Slug akan otomatis dibuat dari nama addon.</span>
            </div>

            <div class="form-group">
                <label for="detail">Detail</label>
                <textarea class="form-control {{ $errors->has('detail') ? 'is-invalid' : '' }}" name="detail" id="detail" rows="4">{{ old('detail', '') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stock') ? 'is-invalid' : '' }}" type="number" name="stock" id="stock" value="{{ old('stock', '') }}" min="0">
                @if($errors->has('stock'))
                    <div class="invalid-feedback">
                        {{ $errors->first('stock') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icon') ? 'is-invalid' : '' }}" type="text" name="icon" id="icon" value="{{ old('icon', '') }}" placeholder="Contoh: bi bi-plus-circle / fas fa-plus">
                @if($errors->has('icon'))
                    <div class="invalid-feedback">
                        {{ $errors->first('icon') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" name="is_quantity_based" id="is_quantity_based" class="form-check-input" value="1" {{ old('is_quantity_based', 1) ? 'checked' : '' }}>
