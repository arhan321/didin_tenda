...
<div class="card">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('admin.order-items.index') }}" class="mb-3">
            <div class="row">
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <select name="item_type" class="form-control">
                        ...
                        @foreach($itemTypes as $key => $label)
                            <option value="{{ $key }}" {{ request('item_type') === $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <a href="{{ route('admin.order-items.index') }}" class="btn btn-default">
                        Reset
                    </a>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-order-item">
                <thead>
                    <tr>
                        <th width="10"></th>
                        <th>ID</th>
                ...
                <tbody>
                    @foreach($orderItems as $key => $orderItem)
                        <tr data-entry-id="{{ $orderItem->id }}">
                            ...
                            <td>
                                Rp {{ number_format($orderItem->price ?? 0, 0, ',', '.') }}
                            </td>

                            <td>
                                Rp {{ number_format($orderItem->total_price ?? 0, 0, ',', '.') }}
                            </td>
                            ...
                            <td>
                                ...
                                @can('order_item_delete')
                                    <form action="{{ route('admin.order-items.destroy', $orderItem->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin?');" style="display: inline-block;">
                                        @method('DELETE')
                                        @csrf
...
<script>
    $(function () {
        ...
        let table = $('.datatable-order-item:not(.ajaxTable)').DataTable({
            buttons: dtButtons
        })
